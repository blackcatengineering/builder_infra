...
module "storage-classes" {
  source      = "${var.github_url}/tf-mod-storage-classes"
...
module "builder-istio" {
  ...
  source        = "${var.github_url}/tf-mod-istio"
...
module "builder-jenkins" {
  count       = (var.enable_istio) ? 1 : 0
  source      = "${var.github_url}/tf-mod-jenkins"
  k8s_cluster = openstack_containerinfra_cluster_v1.cluster_1.name
  # domain  = "example.com"
  providers = {
    helm       = helm.helm_config
...
module "builder-ip-masq" {
  ...
  source      = "${var.github_url}/tf-mod-ip-masq-agent"
  k8s_cluster = openstack_containerinfra_cluster_v1.cluster_1.name
  # domain  = "example.com"
  providers = {
    kubernetes = kubernetes.kubernetes_config
...
module "harness" {
  ...
  source = "${var.github_url}/tf-mod-harness"
  providers = {
    helm       = helm.helm_config
    kubernetes = kubernetes.kubernetes_config
  }
...
module "cert-manager" {
  ...
  source = "${var.github_url}/tf-mod-cert-manager"
  providers = {
    helm       = helm.helm_config
    kubernetes = kubernetes.kubernetes_config
  }
...
module "jaeger" {
  ...
  source = "${var.github_url}/tf-mod-jaeger"
  providers = {
    helm       = helm.helm_config
    kubernetes = kubernetes.kubernetes_config
  }
